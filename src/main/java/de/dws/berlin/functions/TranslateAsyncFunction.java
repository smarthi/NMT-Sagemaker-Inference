...
public class TranslateAsyncFunction extends RichAsyncFunction<String[], Tuple2<String,String>> {
  ...
  private InvokeEndpointRequest getInvokeEndpointRequest() {
    InvokeEndpointRequest invokeEndpointRequest = new InvokeEndpointRequest();
    invokeEndpointRequest.setEndpointName(SERVICE_ENDPOINT);
  ...
  private List<Callable<Tuple2<String,String>>> createCallables(String[] strings) {
    final InvokeEndpointRequest invokeEndpointRequest = getInvokeEndpointRequest();
    List<Callable<Tuple2<String,String>>> callableList = new ArrayList<>();
    for (String s : strings) {
      callableList.add(() -> {
        ByteBuffer byteBuffer = ByteBuffer.wrap(s.getBytes());
        invokeEndpointRequest.setBody(byteBuffer);
        return new Tuple2<>(s, (amazonSageMakerRuntime.invokeEndpoint(invokeEndpointRequest)).getBody().toString());
      });
    }
    return callableList;
  }
  ...
  @Override
  public void asyncInvoke(String[] strings, ResultFuture<Tuple2<String,String>> resultFuture) throws Exception {
    List<Future<Tuple2<String,String>>> futures = executorService.invokeAll(createCallables(strings));
